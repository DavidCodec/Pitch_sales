...
import { MessageSquare, ThumbsUp, BarChart3, Smile, Send, X } from 'lucide-react'
			...
			>
				...
				<div className="flex items-center justify-between p-4 border-b">
					...
					<div className="flex items-center space-x-2">
						<Badge variant={conectado ? 'default' : 'destructive'}>{conectado ? 'Conectado' : 'Desconectado'}</Badge>
							...
							<Card>
								...
								<CardContent className="space-y-3">
									<Input
										...
										value={nombreUsuario}
										onChange={(e: any) => setNombreUsuario(e.target.value)}
									/>
									<div className="flex space-x-2">
										<Input
											placeholder="Escribe tu pregunta..."
											value={nuevaPregunta}
											onChange={(e: any) => setNuevaPregunta(e.target.value)}
							...
							{encuestas.map((encuesta: any) => (
								<Card key={encuesta.id}>
									...
									<CardContent className="space-y-2">
										{encuesta.opciones.map((opcion: any) => {
											const votos = encuesta.votos[opcion] || 0
											const totalVotos = Object.values(encuesta.votos).reduce(
